...
import type { GameDTO, SponsorPubliciteDTO, SponsorRecompenseDTO, SponsorRewardRequestDTO } from '../types/api.types';

const sponsorApi = {
  getDashboardStats: () => api.get(SPONSOR_ENDPOINTS.DASHBOARD_STATS),
  listJeux: () => api.get<GameDTO[]>(SPONSOR_ENDPOINTS.JEUX),
  ...
  createPublicite: (data: Record<string, unknown>) =>
    api.post<SponsorPubliciteDTO>(SPONSOR_ENDPOINTS.PUBLICITES, data),
  updatePublicite: (id: number | string, data: Record<string, unknown>) =>
    api.put<SponsorPubliciteDTO>(SPONSOR_ENDPOINTS.PUBLICITE_BY_ID(id), data),
  setPubliciteStatus: (id: number | string, active: boolean) =>
    api.patch<SponsorPubliciteDTO>(SPONSOR_ENDPOINTS.PUBLICITE_STATUS(id), null, { params: { active } }),
